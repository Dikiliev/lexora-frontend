...
interface CreateDialogProps {
    ...
    onChange: (field: keyof OrderFormState) => (event: ChangeEvent<HTMLInputElement>) => void;
}
...
interface RequestChangeDialogProps {
    ...
    onClose: () => void;
    onSubmit: (event: FormEvent<HTMLFormElement>) => void;
    onChange: (field: keyof RequestChangeFormState) => (event: ChangeEvent<HTMLInputElement>) => void;
}
...
export function ChatOrderDialogs({ isClient, createDialog, requestChangeDialog }: ChatOrderDialogsProps) {
    return (
        <>
            {isClient && (
                <Dialog open={createDialog.open} onClose={createDialog.onClose} maxWidth="sm" fullWidth>
                    <Box component="form" onSubmit={createDialog.onSubmit}>
                        <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            Предложить заказ
                            <IconButton onClick={createDialog.onClose} size="small">
                                <CloseIcon />
                            </IconButton>
                        </DialogTitle>
                        <DialogContent dividers>
                            <Stack spacing={2}>
                                ...
                                <TextField
                                    ...
                                    onChange={createDialog.onChange("description")}
                                    required
                                    fullWidth
                                    multiline
                                    minRows={3}
                                ...
                                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                                    <TextField
                                        ...
                                        onChange={createDialog.onChange("source_lang")}
                                        required
                                        fullWidth
                                    />
                                    <TextField
                                        ...
                                        onChange={createDialog.onChange("target_lang")}
                                        required
                                        fullWidth
                                    />
                                </Stack>
                                ...
                                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                                    <TextField
                                        ...
                                        onChange={createDialog.onChange("price")}
                                        required
                                        fullWidth
                                    ...
                                    <TextField
                                        ...
                                        onChange={createDialog.onChange("currency")}
                                        fullWidth
                                    >
            ...
            <Dialog open={requestChangeDialog.open} onClose={requestChangeDialog.onClose} maxWidth="sm" fullWidth>
                <Box component="form" onSubmit={requestChangeDialog.onSubmit}>
                    <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        Изменение условий
                        <IconButton onClick={requestChangeDialog.onClose} size="small">
                            <CloseIcon />
                        </IconButton>
                    </DialogTitle>
                    <DialogContent dividers>
                        <Stack spacing={2}>
                            <TextField
                                ...
                                onChange={requestChangeDialog.onChange("description")}
                                multiline
                                minRows={3}
                                fullWidth
                            />
                            <TextField
                                ...
                                onChange={requestChangeDialog.onChange("volume")}
                                fullWidth
                            />
                            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                                <TextField
                                    ...
                                    onChange={requestChangeDialog.onChange("price")}
                                    fullWidth
                                    inputProps={{ min: 0, step: "0.01" }}
                                />
                                <TextField
                                    ...
                                    onChange={requestChangeDialog.onChange("currency")}
                                    fullWidth
                                >
                                    {["RUB", "USD", "EUR"].map((currency) => (
                                        <MenuItem key={currency} value={currency}>
                            ...
                            <TextField
                                ...
                                onChange={requestChangeDialog.onChange("deadline")}
                                fullWidth
                            />
                        </Stack>
                    </DialogContent>
                    <DialogActions>
                        <Button onClick={requestChangeDialog.onClose}>Отмена</Button>
                        <Button type="submit" variant="contained" disabled={!requestChangeDialog.canSubmit}>
                            Отправить
                        </Button>
                    </DialogActions>
                </Box>
